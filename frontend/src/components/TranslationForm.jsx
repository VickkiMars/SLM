...
export default function TranslationForm({ onSubmitText, onSubmitFile, isProcessing }) {
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col gap-6">
      {/* Main Input Card (Borderless) */}
      <section className="w-full bg-white rounded-3xl p-6 sm:p-7 shadow-xs flex flex-col gap-5">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-[#FFEEDB] text-xs text-[#2E0E00] font-sans font-medium">
            ...
            <div className="m3-segmented-container border-0 bg-[#F0EEEA]">
              <button
                type="button"
        ...
        {activeTab === 'text' ? (
          <form onSubmit={handleTextSubmit} className="flex flex-col gap-4">
            <div className="relative">
              <textarea
                ...
                className="w-full p-4 text-sm font-sans text-[#111111] bg-[#F6F4F0] focus:bg-white focus:ring-2 focus:ring-[#1A56C4]/20 rounded-2xl placeholder:text-[#666666]/60 outline-none resize-y min-h-[140px] transition-all"
              />
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              ...
              <button
                ...
                className="varsity-btn-primary w-full sm:w-auto min-h-[48px] text-xs sm:text-sm font-serif font-extrabold uppercase tracking-wider px-6 py-3 rounded-xl sm:rounded-2xl shadow-2xs flex items-center justify-center gap-2 active:scale-95 focus-visible:ring-2 focus-visible:ring-[#1A56C4] focus-visible:outline-none"
              >
                {isProcessing ? (
                  <>
          ...
          <form onSubmit={handleFileSubmit} className="flex flex-col gap-4">
            ...
            {fileError && (
              <div className="flex items-center justify-between p-3 bg-red-50 text-xs text-red-700 rounded-xl">
            ...
            <div
              ...
              onDrop={handleDrop}
              className={`rounded-2xl p-8 text-center cursor-pointer transition flex flex-col items-center justify-center gap-2 ${
                isDragOver ? 'bg-[#E8EFFF]' : 'bg-[#F6F4F0] hover:bg-[#F0EEEA]'
              }`}
            >
        ...
        <div className="pt-2 flex flex-col gap-3">
          ...
          {showCustomVocab && (
            <div className="flex flex-col gap-2 p-4 bg-[#F0EEEA] rounded-2xl transition-all animate-fadeIn">
              <div className="flex items-center justify-between">
                <label className="text-[10px] font-serif font-extrabold uppercase tracking-wider text-[#666666]">
                  Custom Vocabulary List
                </label>
                <code className="font-mono text-[10px] bg-white px-2 py-0.5 rounded text-[#1A56C4] font-medium">
              ...
              <textarea
                ...
                className="w-full rounded-xl bg-white p-3 text-xs font-mono text-[#111111] outline-none focus:ring-2 focus:ring-[#1A56C4]/20 transition resize-y"
              />
            </div>
          )}
        </div>
