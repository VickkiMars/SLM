...
// ====== In-memory store (Global across warm serverless invocations) ======
...
function storeResult(jobId, value) {
  global.resultStore.set(jobId, value);
  setTimeout(() => global.resultStore.delete(jobId), 86_400_000);
}

const openaiClient = new OpenAI({
  baseURL: process.env.FIKRA_BASE_URL || 'https://api.fikraapi.co.ke/v1',
...
app.post('/api/upload/translate', upload.single('file'), async (req, res) => {
  try {
    ...
    processJob(job); // Trigger processing asynchronously for serverless execution
...
app.post('/api/text/translate', async (req, res) => {
  try {
    ...
    if (!content) return res.status(400).json({ error: 'MISSING_CONTENT', detail: 'The "content" field is required.' });
    ...
    const intervalId = setInterval(() => {
      ...
      try {
        ...
        if (!blob) {
          ...
          if (count > 30) { // Max 60 sec wait per stream in serverless context
